  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
        ...
        <table className="w-full border rounded-lg overflow-hidden">
          ...
          <tbody className="bg-white divide-y">
            {reviews.map((review) => (
              <tr key={review._id} className="hover:bg-gray-50">
                <td className="p-3 font-semibold text-indigo-600">{review.scholarshipName}</td>
                <td className="p-3 font-bold text-pink-800">{review.universityName}</td>
          ...
          {review.createdAt? format(new Date(review.createdAt), "dd/MM/yyyy") : "-"}
                ...
                <td className="p-3 text-center space-x-2">
                  <Button
                    onClick={() => navigate(`/dashboard/update-reviews/${review._id}`)}
      ...
      <div className="hidden md:grid lg:hidden grid-cols-2 gap-4">
        {reviews.map((review) => (
          <div key={review._id} className="border rounded-lg p-4 shadow-sm bg-white space-y-2">
            <h3 className="font-semibold text-indigo-600">{review.scholarshipName}</h3>
            <p className="text-sm font-bold text-pink-800">{review.universityName}</p>
      ...
      <div className="md:hidden space-y-4">
        {reviews.map((review) => (
          <div key={review._id} className="bg-white border rounded-lg shadow-sm p-4 space-y-2">
            <h3 className="font-semibold text-indigo-600">{review.scholarshipName}</h3>
            <p className="text-sm font-bold text-pink-800">{review.universityName}</p>
